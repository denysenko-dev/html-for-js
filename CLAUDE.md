...
`lesson-17` is one snapshot in a per-lesson branch chain (`lesson-1` … `lesson-19`) built for a frontend
...
where this same project sits under `main`'s `unit_17/` instead of the repo root. Don't expect `unit_NN/`
folders on this branch — see `main`'s `CLAUDE.md`/`README.md` for the full course structure.

## Commands

```
npm install    # installs devDependencies (gulp + plugins) and dependencies (bootstrap)
...
- `css` — minifies `src/css/*` with `gulp-clean-css`, rebuilds only changed files (`lesson-15` used the older,
  deprecated `gulp-cssnano` instead — check the branch you're on before assuming which one applies)
