...
`lesson-18` is one snapshot in a per-lesson branch chain (`lesson-1` … `lesson-19`) built for a frontend
...
where this same project sits under `main`'s `unit_18/` instead of the repo root. Don't expect `unit_NN/`
...
- `fonts` — copies a **hardcoded list** of `.woff2` files out of `node_modules/@fontsource/*` (Roboto,
  ...
  must be updated by hand, it isn't derived automatically — added this lesson
